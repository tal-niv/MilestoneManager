...
exports.MILESTONE_CREATION_PATTERN = void 0;
 ...
 */
exports.MILESTONE_CREATION_PATTERN = /^create milestone:\s*(.+)$/i;
/**
 * Register chat commands for milestone manager
 * @param context Extension context
 */
function registerChatCommands(context) {
    // Register text processor for chat
    context.subscriptions.push(vscode.workspace.registerTextDocumentContentProvider('milestone-manager', {
        provideTextDocumentContent(uri) {
            return '';
        }
    }));
    // Register command to create milestone from chat
    context.subscriptions.push(vscode.commands.registerCommand('milestone-manager.chatCreateMilestone', async (milestoneName) => {
        try {
    ...
    context.subscriptions.push(vscode.commands.registerTextEditorCommand('type', async (textEditor, edit, args) => {
        ...
        const match = lineText.match(exports.MILESTONE_CREATION_PATTERN);
        if (match) {
    ...
    context.subscriptions.push(vscode.workspace.onDidChangeTextDocument(async (event) => {
        ...
        for (const change of event.contentChanges) {
            ...
            const match = text.match(exports.MILESTONE_CREATION_PATTERN);
            if (match) {
                const milestoneName = match[1].trim();
                // Wait a bit to let Cursor process the chat message
                setTimeout(async () => {
