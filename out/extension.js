...
class MilestoneManager {
    ...
    async getMilestones() {
        try {
            ...
            const { stdout } = await execAsync('git log --pretty=format:"%H|||%s|||%ad|||%ai" --date=short --grep="MSVE___BR___" -n 100', { cwd: workspacePath });
            if (!stdout.trim()) {
                return [];
            }
            return stdout
    ...
    async getCurrentMilestone() {
        ...
        try {
            ...
            const { stdout: milestoneInfo } = await execAsync(`git log ${currentCommit.trim()} --grep="MSVE___BR___" -n 10 --pretty=format:"%s"`, { cwd: workspacePath });
